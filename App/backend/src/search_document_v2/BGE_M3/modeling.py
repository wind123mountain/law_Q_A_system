...
class BGEM3Model(nn.Module):
    ...
    def load_model(self, model_name, colbert_dim: int = -1):
        ...
        logger.info('loading model-model_name: %s', model_name)
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None, teacher_scores: Tensor = None,
                no_in_batch_neg_flag: bool = None):
        ...
        if self.training:
            if teacher_scores is not None:
                ...
                if no_in_batch_neg_flag is not None and no_in_batch_neg_flag:
                    targets = torch.zeros(q_dense_vecs.size(0), device=q_dense_vecs.device, dtype=torch.long)
                else:
