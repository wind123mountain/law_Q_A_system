...
class SameDatasetTrainDataset(Dataset):
    ...
    """
    def __init__(self, args: DataArguments, default_batch_size: int, seed: int, process_index: int=0, num_processes: int=1):
        self.args = args
...
class EmbedCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        return {"query": q_collated, 
                "passage": d_collated, 
                "teacher_scores": teacher_scores, 
                "no_in_batch_neg_flag": no_in_batch_neg_flag}


class CustomTrainDataset(SameDatasetTrainDataset):
    """Train dataset for BGE model.
    """
    def __init__(self, args: DataArguments, default_batch_size: int, seed: int, process_index: int=0, num_processes: int=1):
        super().__init__(args, default_batch_size, seed, process_index, num_processes)
    ...
    def _create_batch_data(self, batch_raw_data):
        ...
        Returns:
            ...
            List[float]: Teacher scores for model distillation.
        """
        queries, passages, teacher_scores = [], [], None
...
class CustomEmbedCollator(EmbedCollator):
    def __call__(self, features):
        queries = features[0][0]
        passages = features[0][1]
        teacher_scores = features[0][2]
        ...
        queries_inputs = self.tokenizer.batch_encode_plus(
            queries,
            padding=self.padding,
            truncation=True,
            max_length=self.query_max_len,
        ...
        passages_inputs = self.tokenizer.batch_encode_plus(
            passages,
            padding=self.padding,
            truncation=True,
            max_length=self.passage_max_len,
        ...
        return {"query": queries_inputs, 
                "passage": passages_inputs, 
                "teacher_scores": None, 
                "no_in_batch_neg_flag": no_in_batch_neg_flag}